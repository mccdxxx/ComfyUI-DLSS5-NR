...
// v0.3.0-alpha2: temporal mode always supplies a full-resolution R16G16_FLOAT
...
static bool InitNGXSession() {
    ...
    if (g_core_init_project) {
        for (int ver = 0x13; ver <= 0x20 && !core_ok; ++ver) {
            NGXResult r = g_core_init_project(PROJECT_ID, 0, "0.3.0-alpha2", g_runtime_dir.c_str(), g_device.Get(), ver, nullptr);
...
__declspec(dllexport) const char* __cdecl dlss5nr_version() {
    return "0.3.0-alpha2-cpu-staging-nvof";
}
